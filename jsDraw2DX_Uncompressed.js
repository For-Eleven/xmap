...
function jxBrush(color, fillType) {
    ...
    function updateSVG(_svgvmlObj, defs) {
        ...
        if (this.fillType == 'linear-gradient' || this.fillType == 'lin-grad') {
            ...
            stop1.setAttribute('style', 'stop-color:' + this.color.getValue() + ';stop-opacity:0.7');
            stop2.setAttribute('offset', '100%');
            stop2.setAttribute('style', 'stop-color:' + this.color2.getValue() + ';stop-opacity:0.7');
            linearGradient.appendChild(stop1);
            linearGradient.appendChild(stop2);
...
function jxRect(point, point2, pen, brush) {
    ...
    function draw(graphics) {
        var point, scale;
        point = graphics.logicalToPhysicalPoint(this.points[0]);
        point2 = graphics.logicalToPhysicalPoint(this.points[1]);
        ...
        if (!jsDraw2DX._isVML) {
            ...
            _svgvmlObj.setAttribute('width', scale * width);
            _svgvmlObj.setAttribute('height', scale * height);
            _svgvmlObj.style.position = 'absolute'
        }
        else {
            ...
            _svgvmlObj.style.width = scale * width;
            _svgvmlObj.style.height = scale * height;
            _svgvmlObj.style.position = 'absolute';
...
function jxPolyline(points, pen, brush) {
    ...
    function draw(graphics) {
        ...
        for (var i in this.points) {
            points[i] = graphics.logicalToPhysicalPoint(this.points[i]);
        }
        for (var j in points) {
            pointsList = pointsList + points[j].x + ',' + points[j].y + ' ';
        }
...
function jxCircle(center, arc, pen, brush) {
    ...
    function draw(graphics) {
        ...
        arc = graphics.logicalToPhysicalPoint(this.arc);
        scale = graphics.scale;
        ...
        ax = arc.x;
        ay = arc.y;
        var w = Math.abs(cx - ax);
        var h = Math.abs(cy - ay);
        ...
        if (!jsDraw2DX._isVML) {
            ...
            _svgvmlObj.setAttribute('r', scale * radius);
            _svgvmlObj.style.position = 'absolute'
        }
        else {
            var vml = graphics.getVML(), vmlFill;
            ...
            _svgvmlObj.style.width = scale * radius * 2;
            _svgvmlObj.style.height = scale * radius * 2;
            _svgvmlObj.style.position = 'absolute';
            _svgvmlObj.style.top = cy - scale * radius;
            _svgvmlObj.style.left = cx - scale * radius;
        }
    ...
    this.getPoints = getPoints;
    function getPoints() {
        return [this.center, this.arc];
    }
}

//Class to hold information and draw ellipse
//将椭圆的绘制方式改为依赖一个正四边形，椭圆和这个四边形内切
function jxEllipse(point1, point2, pen, brush) {
    ...
    this.draw = draw;
    function draw(graphics) {
        var point1, point2,scale;
        point1 = graphics.logicalToPhysicalPoint(this.point1);
        point2 = graphics.logicalToPhysicalPoint(this.point2);
        scale = graphics.scale;
        ...
        if (!jsDraw2DX._isVML) {
            ...
            _svgvmlObj.setAttribute('cx', cx);
            _svgvmlObj.setAttribute('cy', cy);
            _svgvmlObj.setAttribute('rx', scale * width / 2);
            _svgvmlObj.setAttribute('ry', scale * height / 2);
            _svgvmlObj.style.position = 'absolute'
        }
        else {
            var vml = graphics.getVML(), vmlFill;
            ...
            _svgvmlObj.style.height = scale * height;
            _svgvmlObj.style.position = 'absolute';
            _svgvmlObj.style.top = cy - scale * height / 2;
            _svgvmlObj.style.left = cx - scale * width / 2;
        }
